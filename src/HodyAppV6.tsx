...
const navItems: { id: Exclude<Screen, 'interactions'>; label: string; Icon: LucideIcon }[] = [
  ...
  { id: 'chronicle', label: 'Drby', Icon: Images },
...
function Avatar({ user, size = 'normal' }: {
  user: Pick<ApiUser, 'display_name' | 'profile_photo_data'>
...
function ProfileFacts({ user, compact = false }: { user: ApiUser; compact?: boolean }) {
  return (
    <div className={`v3-profile-facts v4-profile-facts${compact ? ' compact' : ''}`}>
      ...
      <span className={user.drink_preference === 'none' ? 'no-shot' : ''}>
        <strong>{drinkLabel(user)}</strong><small>co ti nalét</small>
      </span>
    </div>
  )
}

function WelcomeScreen({ inviter, onRegister, onLogin }: {
  inviter: string
  onRegister: () => void
  onLogin: () => void
}) {
  return (
    <section className="entry-screen v2-entry">
      <div className="entry-poster">
        ...
        <p className="entry-lead">Najdi člověka, odklikni otázku, pobav sa a telefon zase strč do kapsy.</p>
      ...
      <p className="prototype-warning">Mobil je enom tahák. Hody sa furt odehrávajú mezi lidma.</p>
    </section>
  )
}

function RegisterScreen({ inviter, onBack, onDone }: {
  inviter: string
  onBack: () => void
      ...
      <div className="entry-form-card">
        ...
        <form onSubmit={submit} className="entry-form">
          ...
          <div className="v3-registration-facts v4-registration-facts">
            <label><span>Co seš zač</span><select value={gender} onChange={(event) => setGender(event.target.value as Gender | '')} required><option value="">Vyber sa</option><option value="male">Šohaj</option><option value="female">Děvčica</option></select></label>
            <label><span>Jak seš na tom s tancem</span><select value={danceLevel} onChange={(event) => setDanceLevel(event.target.value as DanceLevel | '')} required><option value="">Přiznaj barvu</option><option value="pro">Mazák od muziky</option><option value="amateur">Dvě levé v čižmách</option><option value="wild">Tož to nějak odskáču</option></select></label>
            <label className="v4-drink-select"><span>Co ti može kdo nalét</span><select value={drinkPreference} onChange={(event) => setDrinkPreference(event.target.value as DrinkPreference | '')} required><option value="">Vyber pitný režim</option><option value="slivovica">Slivovicu, jak sa sluší</option><option value="green">Zelenú nebo něco hodnějšího</option><option value="dark">Fernet či rum, hlavně ať to kope</option><option value="anything">Co naleješ, to vypiju</option><option value="none">Nechcu, díky</option></select></label>
          </div>
          {drinkPreference === 'none' && <p className="v4-no-shot-note"><Wine size={16} /> Dobrá. Na panáka tě přes appku nikdo tahat nebude.</p>}
          ...
          <label className={`v2-photo-required${photo ? ' ready' : ''}`}>
            ...
            <div><strong>{photo ? 'Fotka sedí' : 'Nahraj svoju fotku'}</strong><span>{photoBusy ? 'Štelujem ju…' : 'povinné · foťák nebo galerie'}</span></div>
          ...
          <button className="entry-primary" type="submit" disabled={busy || photoBusy || !photo || !gender || !danceLevel || !drinkPreference}><PartyPopper size={20} /> {busy ? 'Zapisuju tě mezi naše…' : 'Tož su na hodech!'}</button>
        </form>
      </div>
    </section>
  )
  ...
  return (
    <section className="entry-screen form-entry v2-entry">
      ...
      <div className="entry-form-card login-card">
        ...
        <h1>Který ty seš?</h1>
        <p className="form-intro">Najdi sa, napiš heslo a val dovnitř.</p>
        <form onSubmit={submit} className="entry-form">
...
function HomeScreen({ me, people, preferredTargetId, onPickTarget, onRequestShot }: {
  me: ApiUser
  people: ApiUser[]
  preferredTargetId: string
  onPickTarget: (id: string) => void
  onRequestShot: (user: ApiUser) => void
}) {
  ...
  if (!person) {
    return <section className="screen screen-home"><PageHeader eyebrow="Na place je zatím ticho" title="Nikoho ti nemám koho hodit." copy="Až sa přidá další člověk, objeví sa tady." /></section>
  }
  ...
  const finishConversation = async () => {
    ...
    try {
      ...
      setStatus(`Zapsané. +${result.points} bodů. Každá potvrzená otázka přihodila bod navrch.`)
    } catch (reason) {
  ...
  const pickMoment = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setStatus(`Momentka s ${person.display_name} už visí v Drbech.`)
    } catch (reason) {
  ...
  return (
    <section className="screen screen-home">
      ...
      <div className="v2-timer-row">
        ...
        {seconds === 0 && <em>Pět minut fuč. Kecat možete dál.</em>}
      </div>
      ...
      <article className="person-card v2-person-card">
        ...
        <div className="person-copy">
          ...
          <div className="v6-person-name-row">
            <h2>{person.display_name}</h2>
            {shotAllowed
              ? <button className="v6-name-shot" type="button" onClick={() => onRequestShot(person)}><GlassWater size={17} /> Pozvat na panáka</button>
        ...
        {!done ? (
          ...
          <div className="v2-free-mode">
            <Sparkles size={25} />
            <div><strong>Volná zábava.</strong><p>Otázky sú odpracované. Telefon už može zpátky do kapsy.</p></div>
        ...
        {momentPhoto && <div className="v2-moment-preview v4-moment-preview"><img src={momentPhoto} alt="Čerstvá momentka" /><span>Uložené v Hodových drbech.</span></div>}
...
function PeopleScreen({ me, people, onRequestShot }: { me: ApiUser; people: ApiUser[]; onRequestShot: (user: ApiUser) => void }) {
  const [query, setQuery] = useState('')
  const filtered = people.filter((user) => user.id !== me.id && user.display_name.toLowerCase().includes(query.toLowerCase()))

  return (
    <section className="screen">
      <PageHeader eyebrow="Kdo je na place" title="Naši lidi" copy="Tady sa nehrajú otázky. Tady sa hlavně zjišťuje, koho vytáhnút na panáka." />
        ...
        {filtered.map((user) => (
          <article className="person-tile v2-person-tile" key={user.id}>
            ...
            <div className="person-tile-body">
              ...
              {canReceiveShot(user)
                ? <button className="primary-button v6-people-shot" type="button" onClick={() => onRequestShot(user)}><GlassWater size={18} /> Pozvat na panáka</button>
                : <div className="v4-shot-blocked"><Wine size={18} /><span>{user.display_name} má „Nechcu, díky“. Tož respekt.</span></div>}
            </div>
          </article>
        ))}
      </div>
...
function ChronicleScreen({ onOpenInteractions }: { onOpenInteractions: () => void }) {
  ...
  const load = async () => {
    try {
      ...
      setError(reason instanceof Error ? reason.message : 'Drby sa někde zatoulaly.')
    }
  }
  ...
  const pickPhoto = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const imageData = await compressMomentPhoto(file)
      await saveMomentPhoto({ image_data: imageData })
      setStatus('Cvaknuté. Fotka už visí mezi Drbama.')
      await load()
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Fotka sa nepodařila přihodit.')
    } finally {
      setPhotoBusy(false)
    }
  }
  ...
  return (
    <section className="screen">
      <PageHeader eyebrow="Hodové drby" title="Co sa tu semlelo" copy="Kdo došel, kdo sa bavil, kdo koho tahá na panáka a co sa podařilo cvaknút." />
      <div className="v6-drby-actions">
        <button className="primary-button" type="button" onClick={() => cameraInput.current?.click()} disabled={photoBusy}><Camera size={19} /> {photoBusy ? 'Štelujem fotku…' : 'Cvaknút fotku'}</button>
        <input ref={cameraInput} className="v2-hidden-input" type="file" accept="image/*" capture="environment" onChange={pickPhoto} />
        <button className="v2-log-button" type="button" onClick={onOpenInteractions}><BookOpenCheck size={20} /><span><strong>Všecky hovory</strong><small>Kdo s kým a co odklikl</small></span><ChevronRight size={18} /></button>
      </div>
      {status && <p className="v2-status">{status}</p>}
      {error && <p className="entry-error">{error}</p>}

      <div className="v2-chronicle-list">
        {events.length === 0 && !error && <div className="v2-empty"><Images size={28} /><strong>Zatím tu fučí větr.</strong><p>Až sa něco semele, Drby to prásknú.</p></div>}
        {events.map((event) => {
          if (event.type === 'join') {
            return (
              <article className="v2-chronicle-event invite-event" key={`join-${event.id}`}>
                <Avatar user={{ display_name: event.joined_name, profile_photo_data: event.joined_photo_data }} />
                <div><span>{formatStamp(event.created_at)}</span><h2>{event.inviter_name ? `${event.inviter_name} dotáhl ${event.joined_name} na hody!` : `${event.joined_name} dorazil na hody!`}</h2><p>Nová tvář v placu. Tož si ju zapamatujte.</p></div>
                <UserPlus size={23} />
              </article>
            )
          }
          if (event.type === 'photo') {
            return (
              <article className="v4-chronicle-photo" key={`photo-${event.id}`}>
                ...
                <div><span>{formatStamp(event.created_at)}</span><h2>{event.author_name}{event.tagged_name ? ` + ${event.tagged_name}` : ''}</h2><p>Cvakli momentku a nechali ju mezi Drbama.</p></div>
              </article>
            )
          }
          if (event.type === 'shot') {
            return (
              <article className="v2-chronicle-event v6-shot-event" key={`shot-${event.id}`}>
                <Avatar user={{ display_name: event.recipient_name, profile_photo_data: event.recipient_photo_data }} />
                <div><span>{formatStamp(event.created_at)}</span><h2>{event.giver_name} pozval {event.recipient_name} na panáka.</h2><p>Teď je řada na {event.recipient_name}, jestli ho vezme, nebo pošle dál.</p></div>
                <GlassWater size={23} />
              </article>
            )
          }
          if (event.type === 'shot_transfer') {
            return (
              <article className="v2-chronicle-event v6-shot-transfer" key={`transfer-${event.id}`}>
                <Avatar user={{ display_name: event.to_name, profile_photo_data: event.to_photo_data }} />
                <div><span>{formatStamp(event.created_at)}</span><h2>{event.from_name} přehodil panáka na {event.to_name}.</h2><p>Původně ho kupoval {event.giver_name}. Panáková štafeta pokračuje.</p></div>
                <Send size={23} />
              </article>
            )
          }
          return (
            <article className="v2-chronicle-event talk-event" key={`talk-${event.id}`}>
              <Avatar user={{ display_name: event.to_name, profile_photo_data: event.to_photo_data }} />
              <div><span>{formatStamp(event.created_at)}</span><h2>{event.from_name} sa bavil s {event.to_name}.</h2><p>Otázky sú ve výpisu hovorů, ať z Drbů není účetní kniha.</p></div>
...
function InteractionsScreen({ onBack }: { onBack: () => void }) {
  ...
  return (
    <section className="screen">
      <button className="entry-back v2-page-back" type="button" onClick={onBack}><ArrowLeft size={18} /> Zpátky do Drbů</button>
...
function ShotLeaderboard({ title, rows }: { title: string; rows: ShotLeaderboardRow[] }) {
  return (
...
function PointLeaderboard({ rows, meId }: { rows: PointLeaderboardRow[]; meId: string }) {
  return (
    <div className="v6-point-board">
      <div className="v6-board-title"><Medal size={22} /><div><span>Bodový šenk</span><h2>Kdo fakt hraje</h2></div></div>
      <div className="v6-point-list">
        {rows.map((row, index) => (
          <div className={`v6-point-row${row.id === meId ? ' me' : ''}`} key={row.id}>
            <strong>{index + 1}.</strong>
            <Avatar user={{ display_name: row.display_name, profile_photo_data: row.profile_photo_data }} size="small" />
            <span>{row.display_name}</span>
            <em>{row.points} b.</em>
          </div>
        ))}
      </div>
      <p className="v6-score-note">První pokec +5, opakovaný +2 a každá potvrzená otázka +1.</p>
    </div>
  )
}

function ZrudaCard({ overview }: { overview: GameOverview }) {
  const requirement = overview.hodova_zruda.requirements
  const progress = Math.min(100, Math.round((
    Math.min(1, overview.me.points / requirement.points) +
    Math.min(1, overview.me.unique_people / requirement.unique_people) +
    Math.min(1, overview.me.questions / requirement.questions)
  ) / 3 * 100))

  return (
    <article className={`v6-zruda-card${overview.hodova_zruda.earned ? ' earned' : ''}`}>
      <div className="v6-zruda-icon">{overview.hodova_zruda.earned ? <Crown size={30} /> : <Flame size={30} />}</div>
      <div className="v6-zruda-copy">
        <p className="eyebrow">Hodová výzva</p>
        <h2>Hodová zrůda</h2>
        <p>{overview.hodova_zruda.earned ? 'Hotovo. Oficiálně si toho na place stihl až podezřele moc.' : 'Ne popularita. Enom poctivě oběhat lidi a něco s nima probrat.'}</p>
        <div className="v6-zruda-stats">
          <span className={overview.me.points >= requirement.points ? 'done' : ''}><strong>{overview.me.points}/{requirement.points}</strong> bodů</span>
          <span className={overview.me.unique_people >= requirement.unique_people ? 'done' : ''}><strong>{overview.me.unique_people}/{requirement.unique_people}</strong> lidí</span>
          <span className={overview.me.questions >= requirement.questions ? 'done' : ''}><strong>{overview.me.questions}/{requirement.questions}</strong> otázek</span>
        </div>
        <div className="v6-progress"><i style={{ width: `${progress}%` }} /></div>
      </div>
    </article>
  )
}
...
function GameScreen({ me, people }: { me: ApiUser; people: ApiUser[] }) {
  ...
  const load = async () => {
    try {
      const [shotsResult, board, game] = await Promise.all([getPendingShots(), getShotLeaderboard(), getGameOverview()])
      ...
      setStatus(reason instanceof Error ? reason.message : 'Herní účetnica zdechla.')
    }
  }
  ...
  const passOn = async (shot: PendingShot) => {
    ...
    try {
      ...
      setStatus(`Panák přehozený na ${targetName}. Drby si to samozřejmě zapíšú.`)
      await load()
    } catch (reason) {
  ...
  return (
    <section className="screen">
      <PageHeader eyebrow="Hra" title="Body, panáky a pověst" copy="Tady už appka počítá. Na place máš ale furt mluvit s lidma, ne hypnotizovat tabulku." />
      {status && <p className="v2-status">{status}</p>}

      {overview && <ZrudaCard overview={overview} />}
      {overview && <PointLeaderboard rows={overview.leaderboard} meId={me.id} />}

      <h2 className="section-title">Na tebe čeká panák</h2>
      <div className="v2-pending-shots">
        {pending.length ? pending.map((shot) => (
          <article className="v2-shot-card" key={shot.id}>
            <Avatar user={{ display_name: shot.forwarded_by_name || shot.giver_name, profile_photo_data: shot.forwarded_by_photo_data || shot.giver_photo_data }} />
            <div className="v2-shot-copy">
              <span>{formatStamp(shot.created_at)}</span>
              {shot.forwarded_by_name
                ? <><h3>{shot.forwarded_by_name} ti přehodil panáka.</h3><p>Původně ho koupil {shot.giver_name}. Mosíš ho přijmút, nebo ho možeš poslat dál.</p></>
                : <><h3>{shot.giver_name} tě pozval na panáka.</h3><p>Panák není automaticky přijatý. Vezmi ho, nebo ho pošli dál.</p></>}
            </div>
            <div className="v2-shot-actions">
              ...
              {forwardCandidates.length ? <div className="v2-forward-row"><select value={forwardTo[shot.id] ?? ''} onChange={(event) => setForwardTo((current) => ({ ...current, [shot.id]: event.target.value }))}>{forwardCandidates.map((person) => <option key={person.id} value={person.id}>{person.display_name}</option>)}</select><button type="button" onClick={() => passOn(shot)}><Send size={17} /> Pošli dál</button></div> : <small>Není komu přeposlat. Všeci buď zmizeli, nebo nechcú.</small>}
            </div>
          </article>
      ...
      <div className="v2-leaderboards"><ShotLeaderboard title="Nejvíc přijatých panáků" rows={received} /><ShotLeaderboard title="Nejštědřejší duše" rows={generous} /></div>
    </section>
  )
}

function ProfileScreen({ me, onMeChanged, onLogout }: {
  me: ApiUser
  onMeChanged: (user: ApiUser) => void
  onLogout: () => void
}) {
  ...
  const saveProfile = async (event: FormEvent) => {
    ...
    try {
      ...
      setEditing(false)
      setShareStatus('Profil je přepsaný. Drbna může zase spát.')
    } catch (reason) {
  ...
  return (
    <section className="screen">
      <PageHeader eyebrow="Tvoje místo" title="Ty sám" copy="Fotka, taneční pověst, pitný režim, stav a verbovací QR." />
      ...
      <article className="v2-qr-card">
        <div><p className="eyebrow">Dotáhni dalšího</p><h2>Tvoje verbovací cedula</h2><p>Kdo načte tenhle QR, přistane na registraci s tvým ménem v odkazu. Když sa přidá, Drby tě prásknú.</p><button className="primary-button" type="button" onClick={shareInvite}><Share2 size={18} /> Poslat pozvánku</button></div>
...
function ShotIntroModal({ person, busy, onConfirm, onClose }: {
  person: ApiUser
  busy: boolean
  onConfirm: () => void
  onClose: () => void
  ...
  return (
    <div className="v6-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target && !busy) onClose() }}>
      <article className="v6-shot-modal" role="dialog" aria-modal="true" aria-labelledby="shot-title">
        <GlassWater size={34} />
        <p className="eyebrow">Poprvé, tož vysvětlíme pravidlo</p>
        <h2 id="shot-title">Pozvat {person.display_name} na panáka?</h2>
        <p>Panák sa tímhle <strong>nepřijme automaticky</strong>. {person.display_name} ho uvidí ve Hře a mosí dát „Beru ho“. Když nechce, može ho přehodit někomu dalšímu.</p>
        <div className="v6-modal-actions">
          <button type="button" className="secondary-button" onClick={onClose} disabled={busy}>Nech to být</button>
          <button type="button" className="primary-button" onClick={onConfirm} disabled={busy}>{busy ? 'Posílám…' : 'Tož pozvat'}</button>
        </div>
      </article>
    </div>
  )
...
export default function HodyAppV6() {
  ...
  const [shotPrompt, setShotPrompt] = useState<ApiUser | null>(null)
  ...
  const sendShot = async (person: ApiUser) => {
    setShotBusy(true)
    setNotice('')
    try {
      ...
      setShotPrompt(null)
      setNotice(`${person.display_name} dostal pozvánku na panáka. Teď ju mosí přijmút, nebo može panáka poslat dál.`)
    } catch (reason) {
      setNotice(reason instanceof Error ? reason.message : 'Panák sa neposlal.')
    } finally {
      setShotBusy(false)
    }
  }

  const requestShot = (person: ApiUser) => {
    if (!canReceiveShot(person)) {
      setNotice(`${person.display_name} má „Nechcu, díky“. Tož ho necháme na pokoji.`)
      return
    }
    if (window.localStorage.getItem(SHOT_EXPLAINED_KEY) === '1') {
      void sendShot(person)
      return
    }
    setShotPrompt(person)
  }
  ...
  return (
    <main className="app-shell v2-shell">
      ...
      {notice && <button className="v6-global-notice" type="button" onClick={() => setNotice('')}><Sparkles size={17} /><span>{notice}</span><strong>×</strong></button>}
      {screen === 'home' && <HomeScreen me={me} people={people} preferredTargetId={targetId} onPickTarget={setTargetId} onRequestShot={requestShot} />}
      {screen === 'people' && <PeopleScreen me={me} people={people} onRequestShot={requestShot} />}
      ...
      {shotPrompt && <ShotIntroModal person={shotPrompt} busy={shotBusy} onClose={() => setShotPrompt(null)} onConfirm={() => void sendShot(shotPrompt)} />}
    </main>
  )
}
